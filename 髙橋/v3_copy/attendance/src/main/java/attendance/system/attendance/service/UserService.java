...
public class UserService {
    ...
    public User registerUser(User user, RegisterUserRequest req) {
        ...
        // 電話番号チェック
        if (req.getTelephoneNum() != null &&
            !req.getTelephoneNum().matches("\\d+")) {
            throw new RuntimeException(MessageCode.M0010.getMessage());
        }

        // 緊急連絡先チェック
        if (req.getEmergencyNum() != null &&
            !req.getEmergencyNum().matches("\\d+")) {
            throw new RuntimeException(MessageCode.M0011.getMessage());
        }
